...
function CategoryWoman() {
    ...
    const handleBuy = (productId) => {
        ...
        axios.post(`http://localhost:8000/api/cartCreate`, { product_id: productId, quantity: 1 }, { headers })
            .then((response) => {
    ...
    return (
        <div className="container" style={{ backgroundColor: "#f0f0f0", padding: "20px", borderRadius: "10px" }}>
            <h2 className="text-center mt-4 mb-3" style={{ fontFamily: "Arial, sans-serif", color: "#343a40" }}>Suplementos y vitaminas</h2>
            <Row xs={1} md={3} className="g-4">
                {products.map((product) => (
                    <Col key={product.id}>
                        <Card className="p-3" style={{ backgroundColor: "#f8f9fa", border: "none", borderRadius: "15px" }}>
                            <Card.Img variant="top" src={product.image} style={{ borderRadius: "10px", objectFit: 'cover' }} />
                            <Card.Body>
                                ...
                                <Button variant="primary" style={{ backgroundColor: "#dc3545", border: "none" }} onClick={() => handleBuy(product.id)}>PARA HACER UN PEDIDO ESCRIBENOS AL SIGUIENTE ENLACE DE WSP: https://wa.me/524492238799</Button>
                            </Card.Body>
                        </Card>
                    </Col>
                ))}
